...
const MoonPhase: React.FC<MoonPhaseProps> = ({ phase, size = 200, optimized = false }) => {
  const [moonSize, setMoonSize] = useState(size)

  // Make moon size responsive
  useEffect(() => {
    const handleResize = () => {
      // Adjust size based on screen width
      if (window.innerWidth < 640) {
        setMoonSize(Math.min(window.innerWidth * 0.6, 200))
      } else {
        setMoonSize(size)
      }
    }
  ...
  const renderMoonSVG = () => {
    ...
    switch (phase) {
      case "New Moon":
        return (
          <svg width={moonSize} height={moonSize} viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
            {svgFilter}
        ...
        return (
          <svg width={moonSize} height={moonSize} viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <circle
              ...
              stroke="#FDE68A"
              strokeWidth="1"
              filter={optimized ? "url(#glow)" : undefined}
            />
